function [outputArg1,outputArg2] = alignment_pipeline(projectname, experimentname,projectxlsfile,sampleId, viz, doDisplacement, dropUncertainMeasures)
...
%   projectxlsfile - path to excel spreadsheet (first sheet should be
%   called 'Sheet1'), e.g. 'Embryo Samples Mechanics Metadata.xlsx'
%   sampleId - integer, row of sample in projectxlsfile, starting from first
...
if strcmp(getenv('HOSTNAME'),'pyadolla-centos-vm-2'),
    wd = fullfile('/mnt/disks/datadisk1/gsbucket/ehsan/', projectname);
    xlspath = fullfile(wd,projectxlsfile);
end

if strcmp(getenv('COMPUTERNAME'),'GPDC2-CF0'),
    wd = fullfile('W:\ehsan\analysis\', projectname);
    xlspath = fullfile(wd,projectxlsfile);
end
...
results_path = fullfile(wd, 'results',experimentname);
...
experiments_path = fullfile(wd,'experiments',experimentname);
%fixednuclei_img_fpath = fullfile(experiments_path, 'fixed_morphology_images','staining',fixednucleifname);
fixednuclei_img_fpath = fullfile(results_path, 'fixed_morphology_images','staining',fixednucleifname);
fixedmembrane_img_fpath  = fullfile(experiments_path, 'fixed_morphology_images','staining', fixedmembranefname);
livenuclei_img_fpath = fullfile(results_path, 'mechanical_measurements','mouse_embryo_20210505','live_images',livenucleifname);
membrane_segmentation_fpath = fullfile(experiments_path,'fixed_morphology_images','segmented_embryo','tiff','membrane',membrane_segmentation_fname);
...
mechanical_measurements_xls_fpath = fullfile(results_path,'mechanical_measurements','mouse_embryo_20210505','mechanical_measurements',mechxlsfname);
...
cellcounts_table_fpath = fullfile(experiments_path,'expression_measurements',cellcount_table_fname);
...
writetable(cellxgene, fullfile('./tmp/',['cell_counts_table_' samplename '_with_msd.txt'])) 
