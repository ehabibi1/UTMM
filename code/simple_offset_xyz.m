function[stackBr,offsets] = simple_offset_xyz(stackA, stackB)
  max_y = max(size(stackA,1),size(stackB,1));
  max_x = max(size(stackA,2),size(stackB,2));
...
% function[stackBr offsets zs] = simple_offset_xyz(stackA, stackB)
%     
...
%     offsets = [xoffset,yoffset,zoffset];
%     zs = [zbegin_A, zend_A, zbegin_B, zend_B];
% end